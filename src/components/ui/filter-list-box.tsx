...
export default function ListBox({ options }: { options: Option[] }) {
  ...
  const hasQueryKey = searchParams?.get('price');
  // const currentSelectedItem = hasQueryKey
  //   ? options.find((o) => o.value === hasQueryKey) // Query params se derive
  //   : { name: '', value: '' }; // Default empty object
  
  const currentSelectedItem: Option = hasQueryKey
  ? options.find((o) => o.value === hasQueryKey) ?? { name: '', value: '' } // Fallback to empty option
  : { name: '', value: '' };
  ...
  useEffect(() => {
    if (hasQueryKey && currentSelectedItem.value !== selectedItem.value) {
      setSelectedItem(currentSelectedItem);
    }
  }, [hasQueryKey, currentSelectedItem, selectedItem.value]);

  function handleItemClick(value: string) {
    setSelectedItem(options.find((o) => o.value === value)!);
    updateQueryparams('price', value); // Query params ko update karein
  }
      ...
      {({ open }) => (
        <div className="relative ltr:ml-2 rtl:mr-2 lg:ltr:ml-0 lg:rtl:mr-0 min-w-[160px]">
          <div className="flex items-center">
            ...
            <Listbox.Button
              className="relative w-full text-sm font-semibold rounded-lg cursor-pointer ..."
          ...
          >
            <Listbox.Options
              static
              className="absolute z-20 w-full py-1 mt-1 overflow-auto text-sm rounded-md shadow-lg ..."
            >
              {options?.map((option, idx) => (
                <Listbox.Option
                  key={idx}
                  className={({ active }) =>
                    `${active ? 'text-brand-dark bg-fill-dropdown-hover' : ''} ...`
                  }
                  value={option}
                >
                  {({ selected, active }) => (
                    <>
                      ...
                      {selected && (
                        <span className={`${active ? 'text-brand' : ''} check-icon absolute inset-y-0 ...`}>
